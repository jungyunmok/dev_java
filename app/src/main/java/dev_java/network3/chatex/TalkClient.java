  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (jtf_msg == obj) {
      ...
      if (row == -1) {// -1=> end of file
        ...
        try {
          oos.writeObject(
              Protocol.WHISPER + Protocol.seperator + nickName + Protocol.seperator + name + Protocol.seperator + msg1);
        } catch (Exception e2) {
          e2.printStackTrace();
        }
      }
      ...
      try {
        oos.writeObject(Protocol.TALK_OUT + Protocol.seperator + this.nickName);
        // 자바가상머신과 연결고리 끊기
        System.exit(0);
      } catch (Exception e2) {
        // TODO: handle exception
