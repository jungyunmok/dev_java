...
public class JDBCTest {
  ...
  // 물리적으로 떨어져있는 오라클 서버에 접속이 가능하다-ojdbc6.jar
  public static final String _DRIVER = "oracle.jdbc.driver.OracleDriver"; // 앞에는 폴더이름, 뒤에는 클래스명
  // 물리적으로 떨어져 있는 오라클 서버에 URL정보 추가
  // 오라클 제품에서는 물리적으로 떨어져있는 서버에 접속하는 방식으로 thin방식과 oci방식을 지원하고있다
  // 그 중 멀티티어 환경에서는 thin 방식이다. 서버의 DNS, 포트번호-1521, SID이름(물리적인 저장소 참조)
